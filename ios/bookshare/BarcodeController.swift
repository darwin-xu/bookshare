...
//  bookshare
//
//  Created by Darwin Xu on 2017/6/21.
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        if supportedBarCodes.contains(metadataObj.type) {
            ...
            if metadataObj.stringValue != nil {
                SwiftyBeaver.debug(metadataObj.stringValue)
                captureSession?.stopRunning()

                //let vc = self.storyboard?.instantiateViewController(withIdentifier: "BookInfo")
                //self.show(vc!, sender: nil)
                self.performSegue(withIdentifier: "showBookInfo", sender: metadataObj.stringValue)
            }
        }
    }
